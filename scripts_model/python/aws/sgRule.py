...
#             # Obter todas as regras do grupo de segurança
#             rules = ec2_client.describe_security_group_rules(
#                 Filters=[{'Name': 'group-id', 'Values': [sg_default_id]}]
#             )['SecurityGroupRules']
...
#             # Filtrar as regras pelo protocolo, porta e CIDR específicos
#             filtered_rules = []
#             for rule in rules:
#                 if (
...
#                 ):
#                     matching_cidrs = [ip_range['CidrIp'] for ip_range in rule.get('IpRanges', []) if 'CidrIp' in ip_range and ip_range['CidrIp'] == cidr_ipv4]
#                     if matching_cidrs:
