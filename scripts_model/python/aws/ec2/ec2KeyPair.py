...
key_pair_path = "G:/Meu Drive/4_PROJ/scripts/scripts_model/python/.default/secrets"
...
if response == 'y':   
    try:
        print("-----//-----//-----//-----//-----//-----//-----")
        print(f"Criando um cliente para o serviço EC2")
        ec2 = boto3.client('ec2', region_name=f'{region}')
        ...
        if existing_key_pairs.get("KeyPairs"):
            print("-----//-----//-----//-----//-----//-----//-----")
            print(f"Removendo o par de chaves criado de nome {key_pair_name}")
            ec2.delete_key_pair(KeyName=key_pair_name)          

            print("-----//-----//-----//-----//-----//-----//-----")
            print(f"Removendo o arquivo de par de chave {key_pair_name}.pem se existir")
            pem_path = os.path.join(key_pair_path, f"{key_pair_name}.pem")
            if os.path.exists(pem_path):
                os.remove(pem_path)
            else:
                print(f"Não existe o arquivo de par de chave {key_pair_name}.pem")

            print("-----//-----//-----//-----//-----//-----//-----")
            print(f"Removendo o arquivo de par de chave {key_pair_name}.ppk se existir")
            ppk_path = os.path.join(key_pair_path, f"{key_pair_name}.ppk")
            if os.path.exists(ppk_path):
                os.remove(ppk_path)
            else:
                print(f"Não existe o arquivo de par de chave {key_pair_name}.ppk")
