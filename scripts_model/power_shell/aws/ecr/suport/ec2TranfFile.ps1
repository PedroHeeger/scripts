...
if ($resposta.ToLower() -eq 'y') {
    ...
    if ((aws ec2 describe-instances --filters "Name=tag:Name,Values=$tagNameInstance" --query "Reservations[].Instances[]").Count -gt 1) {       
        ...
        Write-Output "-----//-----//-----//-----//-----//-----//-----"
        Write-Output "Adicionando as credenciais da AWS ao Docker para acessar o Amazon ECR"
        ssh -i "$keyPairPath\$keyPairName.pem" ubuntu@$ipEc2 "aws ecr get-login-password --region $region | docker login --username AWS --password-stdin $accountId.dkr.ecr.$region.amazonaws.com"
