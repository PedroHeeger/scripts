...
if [ "$(echo "$resposta" | tr '[:upper:]' '[:lower:]')" == "y" ]; then
    ...
    if [[ $(aws ec2 describe-key-pairs --query "KeyPairs[?KeyName=='$keyPairName']" --output json | jq length) -gt 0 ]]; then
        ...
        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Criando o par de chaves de nome $keyPairName"
...
if [ "${resposta,,}" == 'y' ]; then
    ...
    if aws ec2 describe-key-pairs --query "KeyPairs[?KeyName=='$keyPairName']" --output json | grep -q "$keyPairName"; then
        ...
        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Removendo o par de chaves de nome $keyPairName e os arquivos pem e ppk"
        aws ec2 delete-key-pair --key-name "$keyPairName"

        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Verificando se existe o arquivo de par de chaves de nomes de nome $keyPairName.pem"
        if [ -e "$keyPairPath/$keyPairName.pem" ]; then
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Removendo o arquivo de par de chaves de nome $keyPairName.pem"
            rm "$keyPairPath/$keyPairName.pem"
        else
            echo "Não existe o arquivo de par de chaves de nome $keyPairName.pem"
        fi

        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Verificando se existe o arquivo de par de chaves de nome $keyPairName.ppk"
        if [ -e "$keyPairPath/$keyPairName.ppk" ]; then
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Removendo o arquivo de par de chaves de nome $keyPairName.ppk"
            rm "$keyPairPath/$keyPairName.ppk"
        else
            echo "Não existe o arquivo de par de chaves de nome $keyPairName.ppk"
        fi
