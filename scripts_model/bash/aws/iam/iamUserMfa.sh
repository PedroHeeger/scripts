...
if [ "$(echo "$resposta" | tr '[:upper:]' '[:lower:]')" == "y" ]; then
    ...
    mfa_devices=$(aws iam list-mfa-devices --user-name $iamUserName --query "MFADevices[].UserName[]" --output text)
...
if [ "$(echo "$resposta" | tr '[:upper:]' '[:lower:]')" == "y" ]; then
    echo "-----//-----//-----//-----//-----//-----//-----"
    echo "Verificando se existe um MFA associado ao usuário do IAM $iamUserName"
    mfaCount=$(aws iam list-mfa-devices --user-name $iamUserName --query "MFADevices[].UserName[]" --output text | wc -l)

    if [ $mfaCount -gt 1 ]; then
        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Listando todas os dispositivos MFA associados ao usuário do IAM $iamUserName"
        aws iam list-mfa-devices --user-name $iamUserName
