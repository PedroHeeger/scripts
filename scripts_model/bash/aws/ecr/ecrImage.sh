...
if [ "$(echo "$resposta" | tr '[:upper:]' '[:lower:]')" == "y" ]; then
    echo "-----//-----//-----//-----//-----//-----//-----"
    echo "Verificando se existe a image de tag $imageTag do repositório $repositoryName"
    if [ $(aws ecr describe-images --repository-name $repositoryName --query "imageDetails[?contains(imageTags, '$imageTag')].imageTags" | wc -l) -gt 0 ]; then
        ...
        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Removendo a image de tag $imageTag do repositório $repositoryName"
        ...
        echo "Não existe a image de tag $imageTag do repositório $repositoryName"
    fi
else
