...
$mfaFile = "mfaTest.png"
$mfaPath = "G:\Meu Drive\4_PROJ\scripts\aws\.default\secrets\mfa"
...
if ($resposta.ToLower() -eq 'y') {
    ...
    if (($condition).Count -gt 0) {
        ...
        $condition = aws iam list-mfa-devices --user-name $iamUserName --query "MFADevices[].UserName[]" --output text
        if (($condition).Count -gt 0) {
            Write-Output "-----//-----//-----//-----//-----//-----//-----"
...
if ($resposta.ToLower() -eq 'y') {
    ...
    if (($condition).Count -gt 0) {
        ...
        if (($condition).Count -gt 0) {
            ...
            Write-Output "Listando todas os dispositivos MFA associados ao usuário do IAM $iamUserName"
            aws iam list-mfa-devices --user-name $iamUserName
        } else {Write-Output "Não existe um MFA associado ao usuário do IAM $iamUserName"}
    } else {Write-Output "Não existe o usuário do IAM $iamUserName"}
} else {Write-Host "Código não executado"}