...
if [[ $resposta =~ ^[Yy]$ ]]; then
    ...
    if [[ $(aws s3api list-buckets --region $region --query "Buckets[?Name=='$bucketName'].Name" --output text | wc -w) -gt 0 ]]; then
        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Verificando se foi configurado as permissões de leitura para entidade everyone da ACL sobre os objetos do bucket $bucketName (Acesso Público)"
        if [[ $(aws s3api get-bucket-acl --bucket $bucketName --query "Grants[?Grantee.URI=='http://acs.amazonaws.com/groups/global/AllUsers' && Permission=='READ'].Permission" --output text | wc -w) -gt 0 ]]; then
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Já foi configurado as permissões de leitura para entidade everyone da ACL sobre os objetos do bucket $bucketName (Acesso Público)"
            ...
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Listando todas as permissões que a entidade everyone da ACL possuí no bucket $bucketName"
            ...
            if [[ $(aws s3api get-bucket-ownership-controls --bucket $bucketName --query "OwnershipControls.Rules[?ObjectOwnership=='BucketOwnerPreferred'].ObjectOwnership" --output text | wc -w) -gt 0 ]]; then
                echo "-----//-----//-----//-----//-----//-----//-----"
                echo "Já foi configurado o proprietário dos objetos no bucket $bucketName para BucketOwnerPreferred"
            ...
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Concedendo permissões de leitura para entidade everyone da ACL sobre os objetos do bucket $bucketName"
            aws s3api put-bucket-acl --bucket $bucketName --acl public-read

            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Listando a configuração de permissões de leitura para entidade everyone da ACL sobre os objetos do bucket $bucketName (Acesso Público)"
            aws s3api get-bucket-acl --bucket $bucketName --query "Grants[?Grantee.URI=='http://acs.amazonaws.com/groups/global/AllUsers'].Permission" --output text 
        fi
    else
...
if [[ $resposta =~ ^[Yy]$ ]]; then
    ...
    if [[ $(aws s3api list-buckets --region $region --query "Buckets[?Name=='$bucketName'].Name" --output text | wc -w) -gt 0 ]]; then
        ...
        if [[ $(aws s3api get-bucket-acl --bucket $bucketName --query "Grants[?Grantee.URI=='http://acs.amazonaws.com/groups/global/AllUsers' && Permission=='READ'].Permission" --output text | wc -w) -gt 0 ]]; then
            ...
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Restringindo as permissões de leitura para entidade everyone da ACL sobre os objetos do bucket $bucketName"
            ...
            if [[ $(aws s3api get-bucket-ownership-controls --bucket $bucketName --query "OwnershipControls.Rules[?ObjectOwnership=='BucketOwnerPreferred'].ObjectOwnership" --output text | wc -w) -gt 0 ]]; then
                ...
                echo "-----//-----//-----//-----//-----//-----//-----"
                echo "Alterando o proprietário dos objetos no bucket $bucketName para BucketOwnerEnforced"
                ...
                echo "-----//-----//-----//-----//-----//-----//-----"
                echo "Não foi configurado o proprietário dos objetos no bucket $bucketName para BucketOwnerPreferred"
            fi
            ...
            echo "-----//-----//-----//-----//-----//-----//-----"
            echo "Não foi configurado as permissões de leitura para entidade everyone da ACL sobre os objetos do bucket $bucketName (Acesso Público)"
        fi
    else
        echo "-----//-----//-----//-----//-----//-----//-----"
        echo "Não existe o bucket $bucketName"
